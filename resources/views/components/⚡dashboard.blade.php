...
new class extends Component
{
    ...
    public function metrics(): array
    {
        ...
        if ($this->isManagement()) {
            ...
            $completedServices = ServiceInstance::where('company_id', $companyId)
                ->where('status', 'completed')
                ->whereMonth('date', now()->month)
                ->whereYear('date', now()->year)
                ->count();
            ...
            $collabInstancesThisMonth = ServiceInstance::where('company_id', $companyId)
                ->where('status', 'completed')
                ->whereMonth('date', now()->month)
                ->whereYear('date', now()->year)
            ...
            $unpaidInstances = ServiceInstance::where('company_id', $companyId)
                ->where('status', 'completed')
                ->where('payout_status', 'unpaid')
                ->whereHas('users', fn($q) => $q->where('users.id', $userId))
                ->with('users')
    ...
    public function topCollaborators()
    {
        ...
        $companyId = auth()->user()->company_id;
        $collaborators = User::where('company_id', $companyId)->get();
        
        $completedInstancesThisMonth = ServiceInstance::where('company_id', $companyId)
            ->where('status', 'completed')
            ->whereMonth('date', now()->month)
            ->whereYear('date', now()->year)
            ->with('users')
